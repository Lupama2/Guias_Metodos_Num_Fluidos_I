...
Tfin=5;   # Tiempo final
n=2001;     # Instantes de tiempo donde se evaluará la solución
...
#    Caso no lineal
    ...
    uaprox=u(i-1,:);
    errores=1;
    while (j<1000 & errores > 10^-8)  
      delta1 = feval(func,uaprox', t(i));
      u(i,:) = u(i-1,:) + h/2 *(delta1+feval(func,u(i-1,:)', t(i-1)))*0.3;
      j=j+1;
      errores=norm(u(i,:)-uaprox);
      uaprox=u(i,:);
    endwhile
...
u=CN("myf",u0, t);
# Solucion Exacta 
tex=linspace(Tini, Tfin, n);
for i=1:n
 uex(i)=abs((cos(log(u(i,1)))/u(i,1)*Tfin/(n-1)));
endfor
#plot(t,u(:,1),'linestyle','-','or','color','b')
plot(t,u(:,1)*50,'or','color','r','linewidth',3,tex,uex,'linewidth',1);
