...
Re = 1000.0; #X numero de Re basado en Utop L / nu
...
n1 = 30; #Numero de volumenes por direccion
dt=0.2; #delta tiempo adimensional t*U/L
Ndeltat=300; #numero de pasos de tiempo
...
#---------------------------------------------
#---------------------------------------------
Utop1 = cos((k-1)*dt)  # condición de borde periódica
#---------------------------------------------
...
# aplico CN (comentar para EI)
uvel = uvel + b_u*dt;
vvel = vvel + b_v*dt;

for lsimpler=1:3     #Este es el loop de iteraciones simpler!!!

#---------------------------------------------
# Cálculo de velocidades en los bordes de los Volumenes, se usa atrasada para eliminar la parte no lineal
#---------------------------------------------
...
j=n1+1;
usouth(i,j)=0; # hasta acá todo 0 en t=0
for i=2:n1-1
  j=1;
...
#---------------------------------------------
# Calculo de las fuentes diferidas debido al upwind para DC2 (si se comentan es solo UP1)
#---------------------------------------------
...
for j=2:n1
  if (uwest(i,j)>0)
    usourcew(i,j)=(uvel(i,j))/2*dx*uwest(i,j);  # termino no lineal
  else
    usourcew(i,j)=(-uvel(i,j))/2*dx*uwest(i,j);  # uwest vel de adveción
  endif 
  if (usouth(i,j)>0)
...
#---------------------------------------------
# Calculo de las fuentes diferidas debido al upwind para QUICK
#---------------------------------------------
#   PARA VOLUMENES U
##i=1;
##j=1;
...
##usources(i,j)=0;
##for j=2:n1
##  if (uwest(i,j)>0)  # fij = 1
##    usourcew(i,j)=(uvuvelel(i,j))/2*dx*uwest(i,j);  # termino no lineal de borde
##  else
##    usourcew(i,j)=(-uvel(i,j))/2*dx*uwest(i,j);  # uwest vel de adveción
##  endif 
##  if (usouth(i,j)>0)
...
##endfor
##
### "Centro"  - QUICK solo para i entre 3 y n-2 ^ j entre 3 y n-1
##  i=2;
##  j=1;
##  if (uwest(i,j)>0)
##    usourcew(i,j)=(uvel(i,j)-uvel(i-1,j))/2*dx*uwest(i,j);
...
##    endif
##    
##  for j=3:n1-1  # loop de centro original para j -> acá implemento QUICK
##    if (uwest(i,j)>0)
##      usourcew(i,j)=(1/8)*(-uvel(i-2, j)-2*uvel(i-1,j)+3*uvel(i,j))*dx*uwest(i,j);
##    else
##      usourcew(i,j)=(1/8)*(3*uvel(i-1, j)-2*uvel(i,j)-uvel(i+1,j))*dx*uwest(i,j);
##    endif 
##    if (usouth(i,j)>0)
##      usources(i,j)=(1/8)*(-uvel(i, j-2)-2*uvel(i,j-1)+3*uvel(i,j))*dx*usouth(i,j);
##    else
##      usources(i,j)=(1/8)*(3*uvel(i, j-1)-2*uvel(i,j)-uvel(i,j+1))*dx*usouth(i,j);
##    endif  
##  endfor
##  
##  j=n1;
...
##
##
##for i=3:n1-1  # loop principal de QUICK, solo para i entre 3 y n-1 ^ j entre 3 y n-2
##  j=1;
##  if (vwest(i,j)>0)
##    vsourcew(i,j)=(vvel(i,j)-vvel(i-1,j))/2*dx*vwest(i,j);
##  else
...
##  endif  
##    
##  for j=3:n1-2 ## QUICK LOOP
##    if (vwest(i,j)>0)
##      vsourcew(i,j)=(1/8)*(-vvel(i-2, j)-2*vvel(i-1,j)+3*vvel(i,j))*dx*vwest(i,j);     
##    else
##      vsourcew(i,j)=(1/8)*(3*vvel(i-1, j)-2*vvel(i,j)-vvel(i+1,j))*dx*vwest(i,j);
##    endif 
##    if (vsouth(i,j)>0)
##      vsources(i,j)=(1/8)*(-vvel(i, j-2)-2*vvel(i,j-1)+3*vvel(i,j))*dx*vsouth(i,j);          
##    else
##      vsources(i,j)=(1/8)*(3*vvel(i, j-1)-2*vvel(i,j)-vvel(i,j+1))*dx*vsouth(i,j);     
##    endif     
##  endfor
##  
##  j=n1-1;
...
#---------------------------------------------
# Cálculo de los coeficientes de la ecuacion de momento en x, todo del lado izquierdo salvo bu(i,j)
#---------------------------------------------
...
# acá puedo cambiar a CN -> + b_v, pero es un poco molesto, es más fácil alterar la solución (guess) de divergencia libre
MMyS=sparse(MMy);
v=MMyS\(buy);
...
uvel1(k)=(uvel(i1,j1)+uvel(i1+1,j1)+uvel(i1,j1+1)+uvel(i1+1,j1+1))/4; # vel en (0.5;0.5) para n=30
vvel1(k)=(vvel(i1,j1)+vvel(i1+1,j1)+vvel(i1,j1+1)+vvel(i1+1,j1+1))/4; 
...
fprintf(fid, "%e %e %e %e %e %e\n", tiempo, masa(k),derivadatuvel(k),derivadatvvel(k),uvel1(k), vvel1(k))
